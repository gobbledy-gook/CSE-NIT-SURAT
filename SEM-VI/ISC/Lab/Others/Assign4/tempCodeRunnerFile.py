...
def playfair_encrypt(plaintext, key):
    ...
    for i in range(0, len(plaintext), 2):
        ...
        if row1 == row2:
            ...
            ciphertext += playfair_matrix[row1][(col1 + 1) % 6] + playfair_matrix[row2][(col2 + 1) % 6]
        elif col1 == col2:
            # Same column: shift downwards (circular)
            ciphertext += playfair_matrix[(row1 + 1) % 6][col1] + playfair_matrix[(row2 + 1) % 6][col2]
        else:
...
def playfair_decrypt(ciphertext, key):
    ...
    for i in range(0, len(ciphertext), 2):
        ...
        if row1 == row2:
            ...
            plaintext += playfair_matrix[row1][(col1 - 1) % 6] + playfair_matrix[row2][(col2 - 1) % 6]
        elif col1 == col2:
            # Same column: shift upwards (circular)
            plaintext += playfair_matrix[(row1 - 1) % 6][col1] + playfair_matrix[(row2 - 1) % 6][col2]
        else:
